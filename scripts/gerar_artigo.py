"""
gerar_artigo.py — Fase 4 do pipeline diário
...
def gerar_slug(titulo: str) -> str:
    slug = titulo.lower()
    slug = slug.replace("ã", "a").replace("â", "a").replace("á", "a").replace("à", "a")
    slug = slug.replace("ê", "e").replace("é", "e").replace("è", "e")
    slug = slug.replace("í", "i").replace("ì", "i")
    slug = slug.replace("ô", "o").replace("ó", "o").replace("õ", "o").replace("ò", "o")
    slug = slug.replace("ú", "u").replace("ù", "u")
...
SYSTEM_PROMPT = """Você é um especialista em direito e contabilidade aplicados ao mercado de fintechs e serviços financeiros digitais no Brasil, escrevendo para o blog SAFIE Fintechs (fintechs.safie.blog.br).

Seus artigos seguem o estilo SAFIE: técnico, direto, acessível para founders, gestores e profissionais do setor financeiro (não apenas advogados), sem juridiquês excessivo, sem clichês como "no mundo dinâmico" ou "cada vez mais".
...
1. FORMATO BLUF (conclusão primeiro): a PRIMEIRA frase do "resumo_executivo" responde
   ...
   Contexto e ressalvas vêm nas frases seguintes. Errado: "Nos últimos anos, a regulação
   tem mudado...". Certo: "A Resolução X do Bacen passa a exigir Y das instituições de
   pagamento a partir de Z."
...
2. KEY TAKEAWAYS: o campo "key_takeaways" traz de 3 a 5 fatos-âncora citáveis do artigo.
   ...
   valor ou entidade. Errado: "É importante entender as mudanças da nova regulação."
   Certo: "A Resolução BCB 96/2021 disciplina o arranjo de pagamentos instantâneos e
   se aplica a instituições participantes do PIX."
...
5. CITAÇÃO DE SÓCIO: o campo "citacao_socio" traz uma análise em primeira pessoa
   ...
   Alterne entre "Lucas Mantovani" (regulatório, contratos, dados) e "Ítalo Cunha"
   (tributário, financeiro), conforme o tema.
...
Apresente a notícia como ponto de partida e aprofunde a análise jurídica e contábil."""
    ...
    return f"""Escreva um artigo completo para o blog SAFIE Fintechs sobre o tema "{tema_nome}".
...
{{
  ...
  "contexto_juridico": "(3 a 4 parágrafos explicando o que isso significa do ponto de vista jurídico e regulatório brasileiro, com referências a leis e normas específicas, em HTML; comece com <p>; subtítulos internos em <h3>, NUNCA <h2>; use <ul>/<ol>/<table> conforme as REGRAS GEO)",
  "titulo_impacto": "(H2 da seção de impacto prático, denso em entidades, máximo 80 caracteres)",
  "impacto_pratico": "(2 a 3 parágrafos sobre o impacto prático para fintechs, bancos digitais e profissionais do setor financeiro, em HTML com tags <p>)",
  ...
  "referencias": ["{referencia.strip()}" (inclua a fonte original se houver, e outras fontes relevantes)]
}}

Regras:
- O título deve ter no máximo 60 caracteres
- A meta_description deve ter no máximo 155 caracteres
- O FAQ deve ter entre 3 e 5 perguntas reais que founders de fintechs fariam ao ChatGPT ou ao Google sobre o tema
- Todo conteúdo em português brasileiro
- Não use travessão (—)
- Parágrafos curtos: máximo 3 linhas por parágrafo. Prefira dividir uma ideia longa em 2 parágrafos distintos. Facilita a leitura em mobile
...
_AUTHOR_DEFAULT_BLOG = "Lucas Mantovani"
...
def montar_artigo_completo(dados_claude: dict, noticia: dict, config_blog: dict) -> dict:
    ...
    url_blog  = config_blog.get("url_completa", "https://fintechs.safie.blog.br")
    blog_nome = config_blog.get("nome", "SAFIE Fintechs")
    ...
    corpo_html = (
        ...
        f"\n\n<h2>{_h2('titulo_impacto', 'Impacto prático')}</h2>\n" +
